...
class SplashState extends State<Splash>{
  ...
  checkFirstSeen() async {
    ...
    if(_seen){  //봤으면 로그인 스크린 실행, Login.dart
      checkHavingCookie();
      // Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (BuildContext context) => LoginApp()));
    }else{  //안 봤으면 앱 소개 스크린 실행, introduce.dart
      prefs.setBool('seen', true);  
      Navigator.of(context).pushReplacement(MaterialPageRoute(builder: (BuildContext context) => IntroduceScreenApp()));
    }
  }
  ...
  Widget build(BuildContext context) {
    return new SplashScreen(
      ...
      navigateAfterSeconds: new Timer(new Duration( milliseconds: 3000),() =>{checkFirstSeen()} ),
      title : new Text('유한 누리',
        style: new TextStyle(
      ...
      gradientBackground: new LinearGradient(colors: [Color.fromRGBO(0, 105, 97, 1), Color.fromRGBO(86, 187, 182, 1)], begin: Alignment.topLeft, end: Alignment.bottomRight ),
